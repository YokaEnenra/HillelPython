...
def inserter(index: int, array: list, elem):
    """Эта ф-кция получает индекс элемента, список элементов и любое значение, затем добавляет полученное значение в
    список по указанному индексу, затем переиндекесирует и возвращает список """
...
def remover(index: int, array: list):
    """Эта ф-кция принимает индекс и списко элементов, а затем удаляет элемент по указанному индексу,
    переиндексирует и возвращает список """
...
def uniq_counter(list1: list, list2: list):
    """Эта ф-кция принимает два списка и затем указывает сколько уникальных элементов в этих двух списках"""
